...
mongoDB의 데이터 조작어(DML - Data Manipulation Language)는 SQL 문법을 사용하지 않고 자바스크립트 기반의 명령어와 JSON 도큐먼트를 인자로 사용한다. 관계형 데이터베이스의 데이터 조작어와 형태만 다를 뿐 비슷한 역할을 수행한다.
...
mongoDB에서의 데이터 모델링은 업무 성격에 맞는 도큐먼트 구조의 설계가 중요하다. 크게 임베디드 방식과 레퍼런스 방식으로 나뉘며 아래 표와 같다.
